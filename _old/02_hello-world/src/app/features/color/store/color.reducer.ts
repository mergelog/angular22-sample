...
import { ColorActions, ColorPickerFormActions } from './color.actions';
...
export const colorFeature = createFeature({
  ...
  reducer: createReducer(
    ...
    // 画面から直接色を選んだ場合。Effect を経由しないので同期状態は待機中に戻す。
    // 同じ backgroundColor を Effect と画面の 2 経路が更新するが、
    // どちらも「アクション → reducer」を通るので、更新の出所はログで追える。
    on(ColorPickerFormActions.colorPicked, (state, { color }) => ({
      ...state,
      backgroundColor: color,
      syncStatus: 'idle',
    })),
